...
A diferença pro `07-eslint-complete-setup.md` é o que cada um entrega. O 07 descreve os padrões em prosa e deixa o
...
`09-file-size-refactor.md` — é ele que manda o
...
- **`[PACKAGE_MANAGER]`** — `npm`, `pnpm`, `yarn` ou `bun`. Se não souber,
  deixe `npm` e o agente corrige quando lê o projeto.
...
- O raciocínio completo por trás dessas escolhas — por que aviso vira
  ...
  `../tools/06-eslint-biome-quality-gates.md`.

---

**Fonte:** https://raw.githubusercontent.com/soumatheusgomes/vibe-coding-toolkit/main/docs/prompts/08-eslint-quality-gates-install.md

**Status no projeto MuvLog:** ✅ Já implementado (2026-09-20). ESLint quality gates configurados com MAX_LINES=350. 39 arquivos refatorados. 0 violations.
