...
def get_pending_orders(tenant_id=None, settings=None):
    ...
    query = Order.query.filter(
        ...
        not Order.assigned_to_own_driver  # Sem entregador próprio
    )
...
def optimize_route_order(orders, driver_pos=None):
    ...
    while remaining:
        ...
        for stop in remaining:
            if stop['type'] == 'PICKUP' or stop['type'] == 'DELIVERY' and stop['order_id'] in picked_up_orders:
        ...
        for stop in valid_stops:
            ...
            direction_similarity = dot_product / (mag_stop * mag_centroid) if mag_stop > 0 and mag_centroid > 0 else 0
